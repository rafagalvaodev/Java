...
public class Uri2737Application implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		List<LawyerMinProjection> list = repository.searchOne();

		List<LawyerMinDTO> searchOne =
				list
						.stream()
						.map(LawyerMinDTO::new)
						.collect(Collectors.toList());

		for (LawyerMinDTO lawyerMinDTO : searchOne){
			System.out.println(lawyerMinDTO);
		}
